...
draft: false
title: '[Euraka!]11[To be continued]'
...
3.  第三站：现实的鸿沟 —— “复数不存在”
任务： 我要测量这个粒子出现的概率。 困境：
1. 我的波函数 $\psi$ 是复数（比如$1+i$）。
2. 但是“概率”必须是实数（不可能有 30%+ $20i$ 的概率）。
3. 而且概率必须是非负的（不可能有 -10% 的概率）。 结论 3： $\psi$ 本身不能直接代表概率。我们需要对 $\psi$ 进行一种**改造**，把它变成正实数。
...
1. 把复数波函数 $\psi$ 看作“振幅”。
2. 模仿经典物理，计算它的“强度”。
3. 因为是复数，计算模长的平方（自己乘共轭）：$|\psi|^2 = \psi \cdot \psi^*$。<p>
