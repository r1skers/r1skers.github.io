...
§2 和 §3 各自从一个角度提过 $f_n = n\,\mathbf{1}_{(0,\,1/n)}$ 这串尖塔——Fatou 那边用它示范单边不等号，DCT 那边用它示范"找不到天花板"。Part 6 §4 也把它作为"不是阶梯逼近"的反例预告过。这一节把它**摆在三大定理面前同时受审**，看每条定理面对同一份病例时各自怎么反应——这是 §3 关上盖子前的合景时刻。

**病例回放（一句话每步）**

**逐点极限**：$\forall x\gt 0$，$n$ 一大塔就跑出 $x$ 右边，$f_n(x) = 0$；$x = 0$ 始终在塔外。故 $\lim_n f_n(x) = 0$ **处处成立**。

**单项积分**：每个 $f_n$ 是高 $n$、宽 $\tfrac{1}{n}$ 的矩形，
...
每个 $f_n$ 都规规矩矩，但积分**漏掉了整整 1 个单位的质量**。

**质量去哪了**：没有消失。它**逃到无穷高、无穷瘦的塔里藏起来了**——塔越来越窄、越来越高，挤向 $x=0$。极限那一刻塔变成"无限高、零宽度"，面积 1 全部塞进了一条**零测的竖线**里；积分看不见零测集上的东西，于是 1 单位凭空"蒸发"。

**三大定理逐一反应**

**① MCT：不适用，正确回避**

MCT 要求 $f_n$ 单调递增。但这串尖塔位置和形状都在变（$f_1$ 的塔在 $(0,1)$，$f_2$ 的在 $(0,\tfrac12)$，…），根本不存在 $f_n\le f_{n+1}$ 的关系。

MCT 检查前提 → **发现不单调** → **拒绝适用**。它没说错话，它**根本不保证这种情形**。$\checkmark$
...
Fatou 老老实实给出 $\le$，而且这里是**严格小于**——它精准地告诉你：**"质量可能漏掉，左边会比右边小。"** 它没骗你，只是它能保证的仅此一个方向。

**③ DCT：不适用，找不到天花板**

DCT 要求存在可积的 $g$ 满足 $|f_n|\le g$。在 $x\in(\tfrac{1}{n+1}, \tfrac{1}{n})$ 这一段，最高的塔是 $f_n$，高度 $n\approx \tfrac{1}{x}$。所以 $g(x)\ge\tfrac{1}{x}$ 数量级。但

$$
\int_0^1 \tfrac{1}{x}\,dx = +\infty.
$$

天花板**不可积** → DCT **拒绝适用**。$\checkmark$

**三个定理在同一份病例上的态度**

| 定理 | 检查前提 | 反应 | 对不对 |
|---|---|---|---|
| MCT | 单调？ | 否 → 不适用 | $\checkmark$ 没乱保证 |
| Fatou | 非负？ | 是 → 给 $0\le 1$ | $\checkmark$ 方向精准 |
| DCT | 有可积天花板？ | 否 → 不适用 | $\checkmark$ 没乱保证 |

**关键领悟**：这个病例之所以"病态"，是因为它**同时逃过了 MCT 的单调性和 DCT 的天花板**——这正是两个定理设置这两条前提的原因。**条件不是繁文缛节，每一条都在精确地挡住一类质量逃逸**。

Fatou 是唯一无条件适用的，代价是它只给单向不等式——而那个 $0\le 1$ 的严格不等号，正是"质量逃逸 1 个单位"的**案发现场指纹**。

**一句话收束**

病例不会让定理出错，只会让**前提不满足的定理保持沉默**。三大定理的全部智慧藏在它们的前提里：

- 你给的条件越强（单调 / 有天花板），换来的结论越强（等号）；
- 什么都不给，Fatou 至少保你一个不等号。

这就是测度论"工作"的方式——**不是无脑套公式，而是先验前提、再决定能要回什么**。
