...
令 logits 发生小扰动 $\delta z$，并用 $\delta p=J_s(z)\delta z$ 表示输出变化的线性化部分。实际有限变化为
...
它们不是因为“彼此正交”才自动成为特征向量，而是因为 $J_s$ 在整个 contrast plane 上都等于 $(1/3)I$：平面内每个非零向量都是特征值 $1/3$ 的特征向量，因此可以任取正交基。仅有“不变子空间”这一条件还不够。
