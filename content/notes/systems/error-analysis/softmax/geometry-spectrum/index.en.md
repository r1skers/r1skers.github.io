...
weight: 2
aliases:
  - /notes/systems/error-analysis/softmax/note-error-softmax-2-geometry-spectrum/
---
...
Let the logits receive a small perturbation $\delta z$, and define $\delta p=J_s(z)\delta z$ as the linearized output change. The actual finite change is

\[
s(z+\delta z)-s(z)
=\delta p+O(\|\delta z\|_2^2).
\]
...
These vectors are not eigenvectors merely because they are orthogonal. The restriction of $J_s$ to the entire contrast plane is $(1/3)I$: every nonzero vector in that plane is an eigenvector with eigenvalue $1/3$, so any orthonormal basis works. Invariance of the subspace alone would not suffice.
