...
如果说 Part 0 关注的是：
在线性映射里，哪些方向的信息被保留，哪些方向被压缩，哪些方向几乎看不见；

那么这一篇要继续问的是：
**当这些“弱方向”进入参数估计以后，为什么会进一步表现成参数耦合、弱可辨识，甚至不可辨识。**
...
如果把 Part 0 里的视角借过来，这里其实是在做一件平行的事：

- Part 0 用 $A$、$A^\top A$ 和奇异值，判断线性映射里哪些方向强、哪些方向弱；
- 这一篇则用目标函数的海森矩阵，判断参数空间里哪些方向约束强、哪些方向约束弱。

也就是说，Part 0 里的“弱信息方向”，到这里会重新表现成“弱曲率方向”。
...
- 小奇异值对应的是线性映射里几乎被压扁的方向；
- 进入 $A^\top A$ 后，这些弱方向会因为平方效应显得更弱。

而在这里，海森矩阵承担的是类似角色：
它告诉我们在最优点附近，参数空间里的哪些方向“陡”，哪些方向“平”。

所以可以把两篇的关系简单记成：

- Part 0 关注的是“信息如何通过矩阵被保留或压缩”；
- Part 1 关注的是“这种信息强弱如何在目标函数里表现成曲率强弱”。
...
如果只用一句话把 Part 0 和这里接起来，那就是：

**Part 0 里的“小奇异值方向”，到 Part 1 里会继续表现成“小曲率方向”。**
